...
public class PaymentController {
    ...
    /**
     * Get payment by VNPay transaction ID
     */
    @GetMapping("/vnpay/{transactionId}")
    public ResponseEntity<Payment> getPaymentByVnpayTransactionId(@PathVariable String transactionId) {
        try {
            Payment payment = paymentService.getPaymentByVnpayTransactionId(transactionId);
            log.info("Retrieved payment with VNPay transaction ID: {}", transactionId);
            return ResponseEntity.ok(payment);
            
        } catch (Exception e) {
            log.error("Error retrieving payment with VNPay transaction ID {}: {}", transactionId, e.getMessage());
            return ResponseEntity.notFound().build();
        }
    }
